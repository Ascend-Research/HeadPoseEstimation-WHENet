...
class WHENet:
    ...
    @staticmethod
    def convert_tftrt(dir_model: str, dir_trt: str, batch: int = 1):
        ...
        converter = trt.TrtGraphConverterV2(
            input_saved_model_dir=dir_model,
            conversion_params=conversion_params)
        converter.convert()
        def my_input_fn():
            yield (np.random.normal(size=(batch, 224, 224, 3)).astype(np.float32), )

        converter.build(input_fn=my_input_fn)
        converter.save(dir_trt)
